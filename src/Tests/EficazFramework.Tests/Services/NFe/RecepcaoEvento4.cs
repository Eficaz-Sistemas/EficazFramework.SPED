...
public class RecepcaoEvento4Tests : BaseNFeTests
{
    ...
    public async Task CienciaOperacaoTest(string chave)
    {
        ...
        var result = await client.EnvioEventoAsync(Configuration[""], chave, Schemas.NFe.CodigoEvento.Ciencia, Schemas.NFe.Ambiente.Homologacao, null);
        result.Should().NotBeNull();

        result.RespostaCodigo.Should().Be(""); 
        ...
        result.ResultadoEventos[0].InformacaoEventoRetorno.RespostaCodigo.Should().Be("");
